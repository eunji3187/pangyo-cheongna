...
def fetch_buildings(sigungu_cd: str, max_pages=9999) -> pd.DataFrame:
    """시군구코드로 건축물 표제부 전량 수집"""
    records = []
    page = 1
    while page <= max_pages:
        ...
        try:
            ...
            print(f"  p{page}: {len(item_list)}건 (누계 {len(records)}/{total_count})")
...
def join_to_parcels(df_bld: pd.DataFrame, shp_path: Path, boundary_geojson: str, region: str):
    ...
    # PNU 생성: A1(법정동코드10) + A5(지번)
    # PNU = 법정동코드10 + 산여부1 + 본번4 + 부번4
    gdf["pnu"] = (
        gdf["A1"].fillna("").astype(str).str.zfill(10) +
        "0" +
        gdf["A5"].fillna("").astype(str).str.extract(r"^(\d+)", expand=False).fillna("0").str.zfill(4) +
        "0000"
    )
    ...
    df_bld["pnu"] = (
        ...
        df_bld.get("ji", pd.Series("0", index=df_bld.index)).fillna("0").astype(str).str.zfill(4)
    )
    ...
    out = C.DATA_OUT / f"buildings_{region}.geojson"
    gdf_out.to_file(out, driver="GeoJSON")
    print(f"  [{region}] {out.name} 저장 (공필지율 {vacant_pct}%, 평균용적률 {mean_far})")
...
if __name__ == "__main__":
    ...
    print("\n[판교] 건축물 수집 (성남 분당구 41135)")
    df_pg = fetch_buildings(C.PANGYO_SIGUNGU_CD)
    _, stats_pg = join_to_parcels(df_pg, C.AL_D154_41, "boundary_pangyo.geojson", "pangyo")

    print("\n[청라] 건축물 수집 (인천 서구 28260)")
    df_cr = fetch_buildings(C.CHEONGNA_SIGUNGU_CD)
